...
let streams;
const TAG = {
  REQUEST: "spotivity-request",
  RESPONSE: "spotivity-response",
  DEBUG: "spotivity-debug",
  ERROR: "spotivity-error",
  MESSAGE: "spotivity-message",
};

const errLog = debug(TAG.ERROR);
const msgLog = debug(TAG.MESSAGE);
...
function log(tag, data) {
  ...
  // Log
  debug(tag)(_getLoggableData(data));
}
...
function _getLoggableData(data) {
  ...
  if (typeof to_log === "string") { to_log = to_log.replace("[time]", `Time : ${new Date()}`); }
  return to_log;
}
